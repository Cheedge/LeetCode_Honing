"""
Lesson 16 Greedy Algorithm
Located on a line are N segments, numbered from 0 to N - 1, 
whose positions are given in arrays A and B. For each I (0 ≤ I < N) 
the position of segment I is from A[I] to B[I] (inclusive). The segments 
...
The size of a non-overlapping set containing 
a maximal number of segments is 3. For example, 
possible sets are {0, 2, 3}, {0, 2, 4}, {1, 2, 3} 
or {1, 2, 4}. There is no non-overlapping set with 
four segments.
...
returns the size of a non-overlapping set containing a 
...
@pytest.mark.parametrize("A, B, expected", test_case)
