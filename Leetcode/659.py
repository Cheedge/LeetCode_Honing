...
class Solution:
    def isPossible(self, nums: List[int]) -> bool:
        cnt = Counter(nums)
        # end store subsqence endup with item
        end: Dict[int, int] = Counter()
        for it in nums:
            # when cnt[it] decreased to 0
            if not cnt[it]:
                continue
            # {1:1, 2:1, 3:1} cnt[1]-1=0
            cnt[it] -= 1
            # change end item from it-1 (if exist) to it
            if end[it - 1] > 0:
                end[it - 1] -= 1
                end[it] += 1
            # if it - 1 not exist, check it+1 and it+2
            elif cnt[it + 1] and cnt[it + 2]:
                cnt[it + 1] -= 1
                cnt[it + 2] -= 1
                end[it + 2] += 1
            # others
