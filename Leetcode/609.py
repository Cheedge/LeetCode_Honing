...
with content (f1_content, f2_content ... fn_content) respectively in the directory "root/d1/d2/.../dm".
...
class Solution:
    def findDuplicate(self, paths: List[str]) -> List[List[str]]:
        # d["filename"] = ["path1", "path2"]
        d = defaultdict(list)
        n = len(paths)
        for i in range(n):
            file_content = re.findall(r"\s([^()]*)\(([^(]*)\)", paths[i])
            path_file = paths[i].split(" ")
            path = path_file[0]
            for f, ct in file_content:
                d[ct].append(path + "/" + f)
            # path, n1 = path_file[0], len(path_file)
            # for j in range(1, n1):
            #     file_content = re.findall(r"\s([^()]*)\(([^(]*)\)", path_file[j])
            #     for f, ct in file_content:
            #         d[ct].append(path+"/"+f)
            # for ct in contents:
            #     d[ct].append(path)
            # for j in range(1, n1):
            #     d[path_file[j]].append(path)
        res = list()
        for f, ps in d.items():
            if len(ps) > 1:
                res.append(ps)
