...
class Solution:
    def pseudoPalindromicPaths(self, root: Optional[TreeNode]) -> int:
        # in one brunch comprise: at most 1 odd, others are all pairs
        # DFS check every brunches, if remains at most 1 odd number, it is a palindrome
        def DFS(node: Optional[TreeNode], path: list) -> int:
            ...
            if not node.left and not node.right:
                # print(node.val, path, "inside")
                if len(path) <= 1:
                    return 1
                else:
                    return 0
            # print(node.val, path)
            left = DFS(node.left, list(path))
            right = DFS(node.right, list(path))
            # print(node.val, path, left, right)
            return left + right
