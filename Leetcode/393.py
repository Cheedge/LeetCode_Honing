...
class Solution:
    def validUtf8(self, data: List[int]) -> bool:
        ...
        nums = list()
        for k in range(n):
            nums.append(bin(data[k])[2:].zfill(8))
        while i < n:
            if nums[i].startswith("0"):
                i += 1
            elif nums[i].startswith("110"):
                if i + 1 < n:
                    if nums[i + 1].startswith("10"):
                        i += 2
                    else:
                        return False
                else:
                    return False
            elif nums[i].startswith("1110"):
                if i + 2 < n:
                    if nums[i + 1].startswith("10") and nums[i + 2].startswith("10"):
                        i += 3
                    else:
                        return False
                else:
                    return False
            elif nums[i].startswith("11110"):
                if i + 3 < n:
                    if (
                        nums[i + 1].startswith("10")
                        and nums[i + 2].startswith("10")
                        and nums[i + 3].startswith("10")
                    ):
                        i += 4
                    else:
                        return False
