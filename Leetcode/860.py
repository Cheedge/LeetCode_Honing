...
def lemonadeChange(bills):
    ...
    :rtype: bool
    """
    # 1. loop over bills, find bills[idx]>5
    # 2. from j<idx to find bill which fit bills[idx]-bills[j]-bills[k]...=5
    # 3. set bills[j], bills[k]... = 0, move to idx+1 check next
    n = len(bills)
    if bills[0]>5: return False
    d = defaultdict(list)
    d[5], d[10], d[20] = [], [], []
    for idx in range(n):
        d[bills[idx]].append(idx)
        if bills[idx]>5:
            residual = bills[idx]
            while residual > 0:
                if residual == 20:
                    if len(d[10])>0 and len(d[5])>0:
                        d[10].pop()
                        d[5].pop()
                    elif len(d[10])==0 and len(d[5])>2:
                        d[5].pop()
                        d[5].pop()
                        d[5].pop()
                    else:
